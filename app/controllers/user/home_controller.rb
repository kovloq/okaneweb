class User::HomeController < UsersController
  def index
  	...
  	if Rails.env.development?
  		...
  		@income=Income.select("COUNT(*) as total,category_id").where("MONTH(date) = ? AND member_id = ? ",DateTime.now.strftime("%m"),current_member["id"]).group("category_id")
	  	@expense=Expense.select("COUNT(*) as total,category_id").where("MONTH(date) = ? AND member_id = ? ",DateTime.now.strftime("%m"),current_member["id"]).group("category_id")
  	end
